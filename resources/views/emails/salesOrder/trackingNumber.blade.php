                        ...
                        <td align="center">
                            <h1 style="font-size:1.5rem;font-weight:300;margin:0 0 10px 0;">
                                @if($salesOrder->is_company)
                                    {{ get_phrase('tracking_number_title', ['name' => $salesOrder->billingAddress->full_name ?? '']) }}
                                @else
                                    {{ get_phrase('tracking_number_title', ['name' => $salesOrder->billingAddress->first_name ?? '']) }}
                                @endif
                            </h1>
                            <p style="margin:0 0 20px 0;">{{ get_phrase('tracking_number_text_1') }}</p>

                            <p style="margin:0 0 4px 0;font-weight: bold;">{{ get_phrase('tracking_number_sub_title') }}:</p>
                            <p style="margin:0 0 20px 0;">{{ $trackingNumber }}</p>

                            <p style="margin:0 0 10px 0;">{{ get_phrase('tracking_number_text_2') }}</p>
                        </td>
                    </tr>

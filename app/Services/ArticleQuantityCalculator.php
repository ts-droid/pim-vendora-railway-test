    ...
    public static function getOnOrderQuantities(): array
    {
        ...
        if ($onOrderQuantities === null) {
            ...
            $onOrderQuantities = DB::table('sales_order_lines')
                ...
                ->where('sales_order_lines.is_completed', '=', 0)
                ->whereIn('sales_orders.order_type', ['WO', 'SO'])
                ->whereIn('sales_orders.status', ['Open', 'BackOrder', 'Hold', 'Shipping'])
                ->select('sales_order_lines.article_number', DB::raw('SUM(quantity - quantity_on_shipments) as total_quantity'))

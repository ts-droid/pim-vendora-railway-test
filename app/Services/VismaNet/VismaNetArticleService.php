...
class VismaNetArticleService extends VismaNetApiService
{
    ...
    public function createArticle(Article $article, bool $firstCall = true): void
    {
        ...
        EventLogger::logSync('Article created in Visma.net', [
    ...
    public function updateArticle(Article $article, bool $firstCall = true): void
    {
        ...
        EventLogger::logSync('Article updated in Visma.net', [
            'article_number' => $article->article_number,
            'payload' => $postData
        ]);
    ...
    public function getPostData(Article $article, bool $isNewArticle = false): array
    {
        ...
        if (($article->color_en ?? '') && !($articleAttributes['exclude_color'] ?? false)) {
            $description = $description . ' - ' . $article->color_en;
        }
        $description = preg_replace('/[^a-zA-Z0-9\s\-åäöÅÄÖ()]/u', '', $description);
